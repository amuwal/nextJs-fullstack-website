...
const EditEducationModal = ({ hideModal, educationData, onSave }) => {
  ...
  <div className=" max-h-[25vh] overflow-y-auto">
          ...
          {editedEducations.map((education, index) => (
            <div key={index} className="mb-4 flex align-center justify-between items-center">
              <div className="flex flex-col">
                <p className="text-sm font-semibold mb-1">{education.institute}</p>
                <p className="text-sm">
          ...
          <div className="border p-4 rounded]">
            <input
              ...
              className={`border rounded p-2 w-full mb-2 ${
                validationError && !newEducation.institute.trim() ? 'border-red-500' : ''
              }`}
            />
            ...
            <input
              ...
              onChange={(e) => setNewEducation({ ...newEducation, start: e.target.value })}
              className={`border rounded p-2 w-full mb-2 ${
                validationError && !newEducation.start.trim() ? 'border-red-500' : ''
              }`}
            />
            ...
            <input
              ...
              onChange={(e) => setNewEducation({ ...newEducation, end: e.target.value })}
              className={`border rounded p-2 w-full mb-2 ${
                validationError && !newEducation.end.trim() ? 'border-red-500' : ''
              }`}
            />
            ...
            <input
              ...
              className={`border rounded p-2 w-full mb-2 ${
                validationError && !newEducation.course.trim() ? 'border-red-500' : ''
              }`}
            />
            ...
            <textarea
              ...
              className={`border rounded p-2 w-full mb-2 ${
                validationError && !newEducation.details.trim() ? 'border-red-500' : ''
              }`}
              rows={4}
            />
          ...
          <div className="flex justify-end mt-4">
            <button className="px-4 py-2 bg-green-500 text-white rounded mr-2" onClick={handleSaveClick}>
              Save
            </button>
...
const EducationComponent = ({ user, setUser }) => {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200 m-3">
      ...
      <div>
        {educations?.length ? educations.map((edu, index) => (
          <EducationItem key={index} education={edu} />
        )) : "lol he is illiterate"}
      </div>
      {showModal && (
        <EditEducationModal hideModal={()=>setShowModal(false)} educationData={educations}  onSave={onSave} />
      )}
    </div>
  );
};
